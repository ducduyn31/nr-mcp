...
export class NewRelicNrqlService extends NewRelicBaseService {
   ...
   */
  async queryNrqlsByService(options: DashboardsQueryOptions): Promise<ServiceNrqlQueryResult> {
    try {
      defaultLogger.info(`Querying New Relic NRQLs for service: ${options.serviceName}`);
      ...
      const { results: dashboardNrqlsArrays, errors } = await PromisePool
        .withConcurrency(this.MAX_CONCURRENCY)
        .for(dashboardsResult.dashboards)
        .process(async (dashboard) => {
          try {
            const dashboardDetails = await dashboardsService.getDashboardDetails(dashboard.guid);
            return this.extractNrqlsFromDashboard(dashboardDetails);
          } catch (error) {
            defaultLogger.error(`Error fetching details for dashboard ${dashboard.name} (${dashboard.guid})`, error);
            return [] as NrqlQuery[];
          }
        });
      ...
      if (errors.length > 0) {
        defaultLogger.error(`Encountered ${errors.length} errors while processing dashboards`);
      }
      ...
      defaultLogger.error(`Failed to query NRQLs for service: ${options.serviceName}`, error);
      throw error;
    }
  }
   ...
   */
  private extractNrqlsFromDashboard(dashboardDetails: DashboardDetails): NrqlQuery[] {
