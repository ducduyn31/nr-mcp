...
class TestNewRelicBaseService extends NewRelicBaseService {
  public async testExecuteNerdGraphQuery<T>(
    query: string,
...
describe("NewRelicBaseService", () => {
  ...
  it("should execute NerdGraph queries successfully", async () => {
    ...
    (client.request as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
      mockResponse,
    );

    const result = await service.testExecuteNerdGraphQuery(
      mockQuery,
      mockVariables,
    );
  ...
  it("should handle errors when executing NerdGraph queries", async () => {
    ...
    const client = service.getGraphQLClient();
    (client.request as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(
      mockError,
    );

    await expect(service.testExecuteNerdGraphQuery(mockQuery)).rejects.toThrow(
      mockError,
    );
  });
});
...
class TestNewRelicLogsService extends NewRelicLogsService {
  public mockExecuteNerdGraphQuery<T>(
...
describe("NewRelicLogsService", () => {
  ...
  it("should query logs with options object", async () => {
    ...
    const pastTimestamp =
      currentTimestamp - (options.startTimeRange || 60) * 60 * 1000;
  ...
  it("should handle errors when querying logs", async () => {
    ...
    await expect(service.queryLogs("SELECT * FROM Log")).rejects.toThrow(
      mockError,
    );
  });
});
